            ...
            $notification = \DB::table('notification_channels')
                    ...
                    if ($notification && $notification->mail == 1) {
                        ...
                        $notificationData = [
                            ...
                            'mail_body' =>str_replace(['{name}','{transaction_id}','{currency}', '{amount}', '{current_balance}'], [$user->name,$transaction_id,$currency, $request->amount, $user_wallet->amount_balance], $translation->mail_body ?? $notification->mail_body),
                        ...
                        // Send welcome email
                        // dd($user->email);
                        Mail::to($user->email)->send(new WalletAmountTransferMail($user, $notificationData));
                    }
